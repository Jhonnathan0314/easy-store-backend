...
public class S3ProductService {
    ...
    public Product addFile(S3File s3File, Long productId) throws NoResultsException, NoIdReceivedException,
        ...
        log.info("ACCION ADDFILE PRODUCT -> Inicia carga de archivo para el producto: {}", productId);
        ...
        log.info("ACCION ADDFILE PRODUCT -> Subiendo archivo: {}", s3File);
        ...
        log.info("ACCION ADDFILE PRODUCT -> Actualizando producto: {}", product);
        product = updateProductUseCase.update(product);
        log.info("ACCION ADDFILE PRODUCT -> Finalizo actualizacion de producto");
        return product;
    }
    ...
    public Product deleteFile(S3File s3File, Long productId, Long accountId) throws NoResultsException, NoIdReceivedException, NoChangesException, InvalidBodyException {
        log.info("ACCION DELETEFILE PRODUCT -> Inicia eliminado de archivo para el producto: {}", productId);

        log.info("ACCION DELETEFILE PRODUCT -> Eliminando archivo: {}", s3File);
        s3Service.deleteObject(accountId, "product", s3File.getName());

        log.info("ACCION DELETEFILE PRODUCT -> Ajustando atributos producto");
        Product product = findByIdProductUseCase.findById(productId);
        product.setImageNumber(product.getImageNumber() - 1);
        ...
        log.info("ACCION DELETEFILE PRODUCT -> Actualizando producto: {}", product);
        product = updateProductUseCase.update(product);
        log.info("ACCION DELETEFILE PRODUCT -> Producto actualizado");
        return product;
    }
}
